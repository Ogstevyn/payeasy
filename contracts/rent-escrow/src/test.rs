...
fn test_refund_zeros_balance() {
    let env = Env::default();
    let (client, _, roommate_a, _, token_address) = setup_escrow(&env);
    let token_client = TokenClient::new(&env, &token_address);
    client.contribute(&roommate_a, &500_i128);
    assert_eq!(token_client.balance(&roommate_a), 500_i128);
    client.refund(&roommate_a);
    assert_eq!(client.get_total_funded(), 0_i128);
    assert_eq!(token_client.balance(&roommate_a), 1000_i128);
}
